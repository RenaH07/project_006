...
// 先頭の連番（01..08）を抽出して数値IDへ（先頭以外の数字は無視）
function extractLeadingId(file){
  const basename = String(file).split('/').pop();    // パス切り落とし
  const m = basename.match(/^(\d{2,3})[_-]/);        // 先頭2〜3桁 + 区切り記号
...
async function preloadStimuliList(){
  ...
  if (!Array.isArray(m.main) || m.main.length !== 8) {
    throw new Error(`manifest.main は 8 本の配列である必要があります（現在 ${m.main?.length} 本）。`);
  }
  // 先頭2桁のIDで並び直す（01_..〜08_..想定）
  const files = m.main
    .slice()
    .sort((a,b)=> (a.match(/^(\d{2})/)?.[1]??'99') - (b.match(/^(\d{2})/)?.[1]??'99'))
...
timeline.push({
  ...
  stimulus: `
    ...
    <div style="text-align: left; max-height: 500px; overflow-y: auto; padding: 10px; border: 1px solid #ccc; font-size: 14px;">
      ...
      本調査は15分程度の回答時間を要します。パソコン（Windows、Mac等）を用いて、静穏な環境でご回答ください。</p>

      <h3>報酬のお支払いについて</h3>
      <p>以下の要件をすべて満たした方に、ランサーズ上で指定した金銭報酬をお支払いいたします。<br>
      ...
      本研究についてご質問がある場合も、ランサーズのメッセージ機能でご連絡ください。<br>
...
timeline.push({
  ...
  // ★ 説明を読んでいる間に、練習4本を先読み
  on_load: ()=>{
    prefetchStim('stimuli/t1_g0o0_j0_jp0_n0_np0__rot0__cut0-618.json');
    prefetchStim('stimuli/t2_g1o0_j0_jp0_n0_np0.json');
    prefetchStim('stimuli/t3_g1o0_j40_jp197_n0_np0.json');
    prefetchStim('stimuli/t4_g1o0_j40_jp197_n0p2_np619.json');
    } 
});

// 練習（4本）
const practiceFiles = [
  'stimuli/t1_g0o0_j0_jp0_n0_np0__rot0__cut0-618.json',
  'stimuli/t2_g1o0_j0_jp0_n0_np0.json',
  'stimuli/t3_g1o0_j40_jp197_n0_np0.json',
  'stimuli/t4_g1o0_j40_jp197_n0p2_np619.json'
];
...
// ★ 本番順序：Williams行列で決定（8本を想定）
const files8 = await preloadStimuliList();
const order  = orderByWilliams8(files8);
...
prefetchStim(order[1]);
  ...
  order.forEach((file, idx)=>{
    ...
    const n = idx + 1;
    const isLast = (n === order.length);     // ★ 最後のページだけ IMCを含める
    ...
    (async ()=>{
      ...
      if (ok){
        ...
        jsPsych.endExperiment(
          ...
          '<p class="warn">※このパスコードが正しく入力されない場合、<br>報酬をお支払いできません。</p>' +
